...
from ndae.training import load_resume_checkpoint, load_sample_checkpoint, save_checkpoint
...
def test_save_checkpoint_writes_step_and_latest_layout(tmp_path: Path) -> None:
    ...
    for directory in (checkpoint_dir, latest_dir):
        ...
        assert (directory / "scheduler.pt").is_file()
    ...
    assert meta["saved_during_eval"] is True
    assert meta["cycle_step"] == 0
    assert meta["effective_lr"] == pytest.approx(trainer.optimizer.param_groups[0]["lr"])
    assert "flashlight.pt" in meta["files"]
    assert "scheduler.pt" in meta["files"]
...
def test_resume_checkpoint_round_trip_restores_model_optimizer_and_trainer_state(
    ...
    assert restored_trainer.scheduler.state_dict().keys() == expected_scheduler.keys()
...
def test_load_sample_checkpoint_accepts_non_resume_ready_checkpoint(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    config = make_config(output_root=str(tmp_path), name="workspace", n_init_iter=0)
    trainer = make_trainer(workspace, config=config)
    ...
    payload = load_sample_checkpoint(
        checkpoint_dir,
        trainer.trajectory_model,
        trainer.system.flash_light,
    )

    assert payload["checkpoint_dir"] == checkpoint_dir.resolve()
    assert payload["meta"]["resume_ready"] is False
